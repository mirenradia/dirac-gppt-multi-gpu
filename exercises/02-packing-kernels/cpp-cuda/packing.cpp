...
// Each thread copies one element from the strided source buffer to the
...
// Each thread copies one element from the contiguous source buffer to the
...
int main(int argc, char* argv[]) {
    ...
    double t2 = MPI_Wtime();
    for (int iter = 0; iter < num_iters; iter++) {
        // TODO: Launch pack_kernel to gather d_send into d_send_buf
        //   pack_kernel<<<(packed_size + block_size - 1) / block_size, block_size>>>(
        //       d_send_buf, d_send, num_messages, msg_size, stride);

        // TODO: Synchronize the GPU (cudaDeviceSynchronize) so packing is
        //       complete before the MPI call.

        // TODO: Post MPI_Isend (d_send_buf -> next) and MPI_Irecv
        //       (d_recv_buf <- prev) into a MPI_Request array.

        // TODO: Wait for both requests with MPI_Waitall.

        // TODO: Launch unpack_kernel to scatter d_recv_buf into d_recv
