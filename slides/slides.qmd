...
## Example MPI program (Fortran) {.smaller}
...
```
Note we are using the modern `mpi_f08` module which should be preferred over the
older `mpi` module and `mpif.h` header file.
...
* How you launch MPI programs depends on the system and you should always
  check the system documentation for details.
* Often you can use `mpiexec` or `mpirun` to launch an MPI program[^pmi]:
  ```{.bash code-line-numbers="false"}
  mpiexec -n <num_processes> ./my_mpi_program <options>
  ```
* Running our previous example with 4 processes would look like:
  ```{.bash code-line-numbers="false"}
  mpiexec -n 4 ./hello_mpi
  ```
  and we would expect output similar to
...
[^pmi]: When MPI is launched in a batch job, assuming it has been configured
correctly, it can usually deduce the number of processes and there is no need to
specify `-n <num_processes>`.
...
* Typically, these scripts are used in your MPI launch line:
  ...
  ```
* Huge potential speedups possible: I've observed a 3x speedup recently.
...
1. Implement the `pack_kernel` routine that gathers the non-contiguous messages
   into a contiguous buffer.
1. Implement the `unpack_kernel` routine that scatters a contiguous buffer back
   into the strided layout.
1. In the packing loop, call the necessary packing/unpacking kernels and the
   non-blocking MPI routines to send/receive the packed buffers. Remember to
...
* Code:
  * Fortran
  * MPI parellization
  * OpenACC for GPU acceleration (currently being ported to OpenMP)
:::

::: {.column width="50%"}
<!-- {{< video vmag_vhrez.mp4 width=540 height=540 >}} -->
![Evolution of the velocity magnitude $|\mathbf{v}|$ for a $1024^3$ simulation
of the Carbon-burning shell](media/prompi-vmag-vhrez.mp4)
:::
...
* MPI derived datatypes:
  ...
  * `MPI_Type_vector` is designed to handle this type of memory layout:
  ![Non-contiguous memory layout in an
      `MPI_Type_vector`](media/prompi-mpi-type-vector-memory-layout.svg)
...
![](media/prompi-comms-before.svg){.fragment height=150}
:::
...
* Forced use of GPU-aware MPI:
  ...
  ```
  (analagous to `$!omp target data use_device_addr(send_buf)` for OpenMP)
...
::: {.column width=50%}
![](media/prompi-comms-after.svg){.fragment width=100%}
:::

::: {.column width=50%}
![](media/prompi-performance-improvement.svg){.fragment width=100%}
:::
...
::: {.column width=35%}
![](media/prompi-strong-scaling.png)
:::
...
example[^isc-sc-multi-gpu-tutorial-2]
...
(`CUDA_RT_CALL()`s omitted for brevity.)
...
```{.cpp startFrom="344"}
        ...
        launch_jacobi_kernel(a_new, a, l2_norm_d, (iy_end - 1), iy_end, nx, calculate_norm,
                             push_bottom_stream);
        cudaEventRecord(push_bottom_done, push_bottom_stream)
```
...
```{.cpp startFrom="357"}
            ...
            cudaMemcpyAsync(l2_norm_h, l2_norm_d, sizeof(real), cudaMemcpyDeviceToHost,
                            compute_stream);
```
...
```{.cpp startFrom="357"}
            cudaStreamWaitEvent(compute_stream, push_top_done, 0);
...
```{.cpp startFrom="366"}
        ...
        MPI_Irecv(a_new + iy_end * nx, nx, MPI_REAL_TYPE, bottom, 0, MPI_COMM_WORLD,
                  &requests[0]);
        MPI_Isend(a_new + iy_start * nx, nx, MPI_REAL_TYPE, top, 0, MPI_COMM_WORLD,
                  &requests[1]);
        ...
        MPI_Irecv(a_new, nx, MPI_REAL_TYPE, top, 1, MPI_COMM_WORLD, &requests[2]);
        MPI_Isend(a_new + (iy_end - 1) * nx, nx, MPI_REAL_TYPE, bottom, 1, MPI_COMM_WORLD,
                  &requests[3]);
```

(`CUDA_RT_CALL()`s and `MPI_CALL()`s omitted for brevity.)
...
```{.cpp startFrom="440"}
    ...
    cudaEventDestroy(reset_l2norm_done);
...
```{.cpp startFrom="440"}
    cudaEventDestroy(push_bottom_done);
    cudaEventDestroy(push_top_done);
    cudaStreamDestroy(push_bottom_stream);
    cudaStreamDestroy(push_top_stream);
    cudaEventDestroy(compute_done);
    cudaEventDestroy(reset_l2norm_done);
    cudaStreamDestroy(compute_stream);
```
