...
::: columns
::: {.column width="60%"}
![](media/domain-full.svg){width=40% fig-align="center" fig-alt="Full 2D Jacobi domain before decomposition"}
...
:::
::: {.column width="30%"}
![](media/domain-decomp-y.svg){width=100% fig-alt="Domain decomposed into horizontal slabs with neighbour halo communication"}
:::
:::

## Exercise: Jacobi solver

## Exercise 01: Jacobi solver
...
## Recap: Supercomputer GPU topology

![](media/supercomputer-gpu-topology.svg)
<!-- The inline SVG images are quite long hence included separately -->
<!-- {{< include includes/mapping-mpi-processes.qmd >}} -->


## NUMA, affinity and GPU distribution {.smaller}
...
* It is technically possible to figure out the topology yourself using tools
  such as `hwloc-ls` and `nvidia-smi topo` but it is usually best and easiest to
  check the system documentation to see if one has been provided.
